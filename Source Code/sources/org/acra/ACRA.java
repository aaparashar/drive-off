...
public class ACRA {
    ...
    static class C02871 implements OnSharedPreferenceChangeListener {
        C02871() {
        }
    ...
    public static void init(Application app) {
        ...
        if (mReportsCrashes == null) {
            log.mo899e(LOG_TAG, "ACRA#init called but no ReportsCrashes annotation on Application " + mApplication.getPackageName());
            return;
        }
        SharedPreferences prefs = getACRASharedPreferences();
        try {
            checkCrashResources();
            log.mo897d(LOG_TAG, "ACRA is enabled for " + mApplication.getPackageName() + ", intializing...");
            ...
            log.mo907w(LOG_TAG, "Error : ", e);
        }
        mPrefListener = new C02871();
    ...
    public static ACRAConfiguration getConfig() {
        if (configProxy == null) {
            if (mApplication == null) {
                log.mo906w(LOG_TAG, "Calling ACRA.getConfig() before ACRA.init() gives you an empty configuration instance. You might prefer calling ACRA.getNewDefaultConfig(Application) to get an instance with default values taken from a @ReportsCrashes annotation.");
            }
