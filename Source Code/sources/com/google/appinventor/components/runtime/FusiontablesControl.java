...
public class FusiontablesControl extends AndroidNonvisibleComponent implements Component {
    ...
    private class QueryProcessorV1 extends AsyncTask<String, Void, String> {
        ...
        private String serviceAuthRequest(String query) {
            ...
            try {
                ...
                Sql sql = new Builder(TRANSPORT, JSON_FACTORY, new GoogleCredential.Builder().setTransport(TRANSPORT).setJsonFactory(JSON_FACTORY).setServiceAccountId(FusiontablesControl.this.serviceAccountEmail).setServiceAccountScopes(FusiontablesControl.this.scope).setServiceAccountPrivateKeyFromP12File(FusiontablesControl.this.cachedServiceCredentials).build()).setJsonHttpRequestInitializer(new GoogleKeyInitializer(FusiontablesControl.this.ApiKey())).build().query().sql(query);
                sql.put("alt", "csv");
    ...
    private void showNoticeAndDie(String message, String title, String buttonText) {
        ...
        alertDialog.setButton(buttonText, new C00411());
        alertDialog.show();
    }
    ...
    private HttpUriRequest genFusiontablesQuery(String query) throws IOException {
        ...
        entity.setContentType(UrlEncodedParser.CONTENT_TYPE);
    ...
    public com.google.api.client.http.HttpResponse sendQuery(String query, String authToken) {
        ...
        com.google.api.client.http.HttpResponse response = null;
        try {
            Sql sql = new Builder(AndroidHttp.newCompatibleTransport(), new GsonFactory(), new GoogleCredential()).setApplicationName("App Inventor Fusiontables/v1.0").setJsonHttpRequestInitializer(new GoogleKeyInitializer(ApiKey())).build().query().sql(query);
            sql.put("alt", "csv");
    ...
    private String doPostRequest(String query, String authToken) {
        ...
        try {
            ...
            entity.setContentType(Json.CONTENT_TYPE);
            request.addHeader(HttpHeaders.AUTHORIZATION, AUTHORIZATION_HEADER_PREFIX + authToken);
            request.setEntity(entity);
            try {
