...
public class ClientLoginHelper implements IClientLoginHelper {
    ...
    private static void addGoogleAuthHeader(HttpUriRequest request, String token) {
        if (token != null) {
            ...
            request.addHeader(HttpHeaders.AUTHORIZATION, AUTHORIZATION_HEADER_PREFIX + token);
        }
    }

    private static void removeGoogleAuthHeaders(HttpUriRequest request) {
        for (Header header : request.getAllHeaders()) {
            if (header.getName().equalsIgnoreCase(HttpHeaders.AUTHORIZATION) && header.getValue().startsWith(AUTHORIZATION_HEADER_PREFIX)) {
