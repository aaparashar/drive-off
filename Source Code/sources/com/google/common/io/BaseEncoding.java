...
public abstract class BaseEncoding {
    ...
    private static final class Alphabet extends CharMatcher {
        ...
        Alphabet(String name, char[] chars) {
            ...
            try {
                ...
                byte[] decodabet = new byte[DateTime.TIMEZONE_MASK];
    ...
    @GwtIncompatible("Writer,OutputStream")
    public final OutputSupplier<OutputStream> encodingStream(final OutputSupplier<? extends Writer> writerSupplier) {
        Preconditions.checkNotNull(writerSupplier);
        return new OutputSupplier<OutputStream>() {
            public OutputStream getOutput() throws IOException {
                return BaseEncoding.this.encodingStream((Writer) writerSupplier.getOutput());
            }
        };
    }
    ...
    @GwtIncompatible("Reader,InputStream")
    public final InputSupplier<InputStream> decodingStream(final InputSupplier<? extends Reader> readerSupplier) {
        Preconditions.checkNotNull(readerSupplier);
        return new InputSupplier<InputStream>() {
            public InputStream getInput() throws IOException {
                return BaseEncoding.this.decodingStream((Reader) readerSupplier.getInput());
            }
        };
    }
